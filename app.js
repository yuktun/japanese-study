import {filterReviewDeck,normalizeReviewProgress,resetReviewStatuses,reviewCounts,reviewKeyFor,reviewRecordFor,setReviewStatus,toggleReviewBookmark} from './src/flashcard-review.mjs';

const REVIEW_STORAGE_KEY='jp-study-flashcard-review-progress';
const state={all:[],references:[],lessons:[],scopeDeck:[],deck:[],index:0,revealed:false,again:0,good:0,quickSeen:0,type:'all',direction:'ja-zh',year:null,lesson:null,ratingEnabled:localStorage.getItem('jp-study-rating-options')==='true',reviewFilter:'all',reviewProgress:normalizeReviewProgress(readJsonStorage(REVIEW_STORAGE_KEY,{})),view:'review',library:{query:'',years:[],lessons:[],types:[]}};
...
function resetDeck(shuffle=false){
  ...
  if(shuffle){for(let i=state.scopeDeck.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[state.scopeDeck[i],state.scopeDeck[j]]=[state.scopeDeck[j],state.scopeDeck[i]];}}
  applyReviewFilter();
...
function answerBackHtml(item,japanese,reverse){
  ...
  const questionLanguage=reverse?'zh-HK':'ja';
  const questionReading=!reverse&&item.type==='vocabulary'&&item.kanji?`<p class="answer-reading" lang="ja">${escapeHtml(item.kana)}</p>`:'';
  const main=reverse
...
async function loadConjugation(){
  ...
  try{
    const [forms,verbs,keigo,quick,plain,derived]=await Promise.all(['./data/conjugation/forms.json','./data/conjugation/verbs.json','./data/conjugation/keigo.json','./data/conjugation/quick-reference.json','./data/conjugation/plain-forms.json','./data/conjugation/derived-forms.json'].map(async path=>{const r=await fetch(path,{cache:'no-cache'});if(!r.ok)throw new Error(path);return r.json();}));
    conjugationState.forms=forms;conjugationState.verbs=verbs;conjugationState.keigo=keigo;conjugationState.quick=quick;conjugationState.plain=plain;conjugationState.derived=derived;conjugationState.loaded=true;renderConjugation();
...
function renderConjugation(){
  ...
  $('#conjugation-basic').hidden=conjugationState.section!=='basic';$('#conjugation-reference').hidden=conjugationState.section==='basic';$('#derived-forms-reference').hidden=conjugationState.section!=='basic';$$('[data-conjugation-section]').forEach(button=>button.classList.toggle('active',button.dataset.conjugationSection===conjugationState.section));
  ...
  $('#conjugation-rule-card').innerHTML=`<div><p class="eyebrow">大家的日本語${form.course?` ${form.course}`:''} · 第${form.lesson}課</p><h2 lang="ja">${form.japaneseName}</h2><p>${form.description}</p></div><div class="conjugation-rules">${form.rules.map(rule=>`<section><b>${rule[0]}</b><span>${rule[1]}</span><em lang="ja">${rule[2]}</em></section>`).join('')}</div>${form.relatedNote?`<p class="related-note">${form.relatedNote}</p>`:''}<small class="source-note">來源：${form.sourceFile}</small>`;
...
function renderConjugationReference(){
  ...
  const renderKeigo=(type,label,description)=>{const data=conjugationState.keigo[type];return `<section class="keigo-group"><div class="keigo-heading"><p class="eyebrow">ADVANCED REFERENCE · 大家的日本語${data.course?` ${data.course}`:''} · 第${data.lesson}課</p><h2>${label}</h2><p>${description}</p></div><div class="keigo-cards">${data.cards.map(card=>`<article><h3>${card.title}</h3><p class="keigo-pattern" lang="ja">${card.pattern}</p><p>${card.note}</p><ul>${card.examples.map(example=>`<li lang="ja">${example}</li>`).join('')}</ul></article>`).join('')}</div><small class="source-note">來源：${data.sourceFile}</small></section>`;};root.innerHTML=renderKeigo('honorific','尊敬語','動作の主体に敬意を表す表現。')+renderKeigo('intermediateHonorific','中級Ⅰ・第9課','～している・状態などに使う尊他敬語。')+renderKeigo('humble','謙譲語','自分の動作をへりくだって表す表現。');
}
function renderDerivedForms(){const data=conjugationState.derived.zuni;$('#derived-forms-reference').innerHTML=`<div class="derived-heading"><p class="eyebrow">ADVANCED DERIVED FORMS · 大家的日本語 中級Ⅰ · 第${data.lesson}課</p><h2>派生形</h2><p>${data.description}</p></div><article class="derived-card"><h3 lang="ja">${data.title}</h3><p>${data.steps.join(' → ')}</p><div class="derived-rows">${data.examples.map(([base,zu,zuni])=>`<div><span lang="ja">${base}</span><strong lang="ja">${zu}</strong><strong lang="ja">${zuni}</strong></div>`).join('')}</div><p class="derived-exception" lang="ja"><b>例外：</b>${data.exception}</p><small class="source-note">來源：${data.sourceFile}</small></article>`;}
...
function renderQuickReference(root){
  ...
  const iRows=[['あ段',[['ない形','nai'],['なかった形','nakatta'],['受身形','passive'],['使役形','causative']]],['い段',[['ます形','masu']]],['う段',[['辞書形','dictionary'],['禁止形','prohibitive']]],['え段',[['可能形','potential'],['条件形','conditional'],['命令形','imperative']]],['お段',[['意向形','volitional']]],['音便',[['て形','te'],['た形','ta']]]];
  const iValue=(verb,key)=>key==='masu'?verb.masu:(key==='passive'||key==='causative'?`${verb.forms[key+'Polite']}／${verb.forms[key]}`:verb.forms[key]);
  const selected=groupI.find(verb=>verb.masu===conjugationState.quickVerb)||groupI[0];
  const desktopTable=`<div class="quick-table quick-desktop-table"><div class="quick-table-head"><span></span>${groupI.map(v=>`<b lang="ja">${v.masu}</b>`).join('')}</div>${iRows.map(([row,forms])=>`<div class="quick-row"><h3>${row}</h3>${groupI.map(v=>`<div>${forms.map(([label,key])=>`<small>${label}</small><strong lang="ja">${iValue(v,key)}</strong>`).join('')}</div>`).join('')}</div>`).join('')}</div>`;
  const mobileTable=`<div class="quick-mobile-reference"><div class="quick-verb-selector" role="tablist" aria-label="I組代表動詞">${groupI.map(verb=>`<button class="${verb===selected?'active':''}" data-quick-verb="${verb.masu}" lang="ja">${verb.masu}</button>`).join('')}</div><h3 class="quick-selected-verb" lang="ja">${selected.masu}</h3><div class="quick-mobile-rows">${iRows.map(([row,forms])=>`<section class="quick-vowel-section"><h3>${row}</h3>${forms.map(([label,key])=>`<div class="quick-form-line"><small>${label}</small><strong lang="ja">${iValue(selected,key)}</strong></div>`).join('')}</section>`).join('')}</div></div>`;
  const iTable=`<section class="quick-card"><div class="quick-heading"><p class="eyebrow">I組 · 五段動詞</p><h2>あ・い・う・え・お段 ＋ 音便</h2><p>五つの代表動詞で、ます形からの動きを一目で確認。</p></div>${desktopTable}${mobileTable}<small class="source-note">大家的日本語 · 第14／17／18／19／35／48課</small></section>`;
  const sound=`<section class="quick-card sound-card"><p class="eyebrow">音便 · 第14課／第19課</p><h2>て形・た形の音の変化</h2><div class="sound-grid"><article><b>促音便</b><p>ち／り／い → って／った</p><strong>待って・待った<br>帰って・帰った</strong></article><article><b>鼻音便</b><p>び／み／に → んで／んだ</p><strong>遊んで・遊んだ<br>読んで・読んだ<br>死んで・死んだ</strong></article><article><b>イ音便</b><p>き → いて／いた<br>ぎ → いで／いだ</p><strong>書いて・書いた<br>泳いで・泳いだ</strong></article><article><b>し</b><p>し → して／した</p><strong>話して・話した</strong></article></div><p class="voiced-note">鼻音便・ぎの形では、て → で、た → だ。</p><p class="exception-note"><b>例外：</b>行きます → 行って／行った</p></section>`;
  const compact=(title,verb,labels)=>`<section class="quick-card compact-reference"><p class="eyebrow">${title}</p><h2 lang="ja">${verb.masu[0]}</h2><div>${labels.map(([label,key])=>{const value=key==='masu'?verb.masu:verb.forms[key];return `<span><small>${label}</small><strong lang="ja">${value[0]}</strong>${value[0]!==value[1]?`<em lang="ja">${value[1]}</em>`:''}</span>`;}).join('')}</div></section>`;
  const major=[['ます形','masu'],['辞書形','dictionary'],['ない形','nai'],['なかった形','nakatta'],['て形','te'],['た形','ta'],['可能形','potential'],['条件形','conditional'],['命令形','imperative'],['意向形','volitional'],['受身形','passive'],['使役形','causative'],['禁止形','prohibitive']];
  const irregular=[['辞書形','dictionary'],['ない形','nai'],['なかった形','nakatta'],['て形','te'],['た形','ta'],['可能形','potential'],['条件形','conditional'],['条件形（否定）','conditionalNegative'],['命令形','imperative'],['意向形','volitional'],['受身形','passive'],['使役形','causative']];
  root.innerHTML=`<div class="quick-reference"><div class="quick-intro"><p class="eyebrow">QUICK REFERENCE</p><h2>速查表</h2><p>基本詞形を横断して見る、活用の地図。</p></div>${iTable}${sound}<section class="quick-card exception-card"><b>あります（有）</b><span>ない形 → <strong lang="ja">ない</strong></span><span>なかった形 → <strong lang="ja">なかった</strong></span><small>第17課のない形の特別な扱い。</small></section>${compact('II組 · 一段動詞',eat,major)}<section class="irregular-reference">${compact('III組 · 不規則動詞',come,irregular)}${compact('III組 · 不規則動詞',doVerb,irregular)}</section></div>`;
  $$('[data-quick-verb]').forEach(button=>button.addEventListener('click',()=>{conjugationState.quickVerb=button.dataset.quickVerb;renderQuickReference(root);}));
}
...
document.addEventListener('keydown',event=>{
  ...
  if(event.code==='Space'){event.preventDefault();state.revealed?hideAnswer():revealCard();return;}
  if(state.ratingEnabled&&state.revealed&&['1','2','3'].includes(event.key))rateCard({1:'again',2:'hard',3:'good'}[event.key]);
});
