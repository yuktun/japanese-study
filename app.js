...
const conjugationState={forms:[],verbs:[],keigo:null,quick:null,formId:'dictionary',section:'basic',group:'ALL',query:'',revealed:false,loaded:false};
...
function renderQuickReference(root){
  ...
  const iTable=`<section class="quick-card"><div class="quick-heading"><p class="eyebrow">I組 · 五段動詞</p><h2>あ・い・う・え・お段 ＋ 音便</h2><p>五つの代表動詞で、ます形からの動きを一目で確認。</p></div><div class="quick-table"><div class="quick-table-head"><span></span>${groupI.map(v=>`<b lang="ja">${v.masu}</b>`).join('')}</div>${iRows.map(([row,forms])=>`<div class="quick-row"><h3>${row}</h3>${groupI.map(v=>`<div>${forms.map(([label,key])=>`<small>${label}</small><strong lang="ja">${iValue(v,key)}</strong>`).join('')}</div>`).join('')}</div>`).join('')}</div><small class="source-note">大家的日本語 · 第14／17／18／19／35／48課</small></section>`;
